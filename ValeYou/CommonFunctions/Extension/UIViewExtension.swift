...
extension UIView {
    ...
    func addDashedCircle(strokeColor: UIColor? = nil, lineWidth: CGFloat? = nil, lineDashPattern: [NSNumber]? = nil ) {
        let circleLayer = CAShapeLayer()
        circleLayer.path = UIBezierPath(ovalIn: bounds).cgPath
        circleLayer.lineWidth = lineWidth ?? 4.0
        circleLayer.strokeColor = (strokeColor ?? .white).cgColor//border of circle
        circleLayer.fillColor = UIColor.clear.cgColor//inside the circle
        circleLayer.lineJoin = .round
        circleLayer.lineDashPattern = lineDashPattern ?? [6,3]
        layer.addSublayer(circleLayer)
    }

    
    func addDashedBorder(width: CGFloat? = nil, height: CGFloat? = nil, lineWidth: CGFloat = 2, lineDashPattern:[NSNumber]? = [6,3], strokeColor: UIColor = UIColor.red,radius:CGFloat, fillColor: UIColor = UIColor.clear) {
