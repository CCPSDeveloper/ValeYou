...
public class HomeFragmentVM extends BaseFragmentViewModel {
    ...
    final SingleRequestEvent<List<ProviderNearMe>> providerList = new SingleRequestEvent<>();
    ...
    public SingleRequestEvent<SimpleApiResponse> addFilterEvent = new SingleRequestEvent<>();
    ...
    public void getProviderList(Location currentlocation, int limit, int page, String search) {
        try {
            welcomeRepo.getJobProvider(sharedPref.getUserData().getAuthKey(), String.valueOf(currentlocation.getLatitude()),
                    String.valueOf(currentlocation.getLongitude()), String.valueOf(limit), String.valueOf(page), search)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<ApiResponse<List<ProviderNearMe>>>() {
                @Override
                public void onSubscribe(Disposable d) {
                    compositeDisposable.add(d);
                    providerList.setValue(Resource.loading(null));
                }

                @Override
                public void onSuccess(ApiResponse<List<ProviderNearMe>> apiResponse) {
                    if (apiResponse.getStatus() == HttpURLConnection.HTTP_OK) {
                        providerList.setValue(Resource.success(apiResponse.getData(), apiResponse.getMsg()));
                    } else {
                        providerList.setValue(Resource.warn(null, apiResponse.getMsg()));
                    }
                }

                @Override
                public void onError(Throwable e) {
                    providerList.setValue(Resource.error(null, networkErrorHandler.getErrMsg(e)));
                }
            });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void addFilter(String cat_ids, int rate, int distance, String state, int no_of_job) {
        welcomeRepo.addFilter(sharedPref.getUserData().getAuthKey(), cat_ids, rate, distance, state, no_of_job)
                .subscribeOn(Schedulers.io())
                ...
                .subscribe(new SingleObserver<SimpleApiResponse>() {
                    ...
                    public void onSuccess(SimpleApiResponse simpleApiResponse) {
                        if (simpleApiResponse.getStatus() == HttpURLConnection.HTTP_OK) {
                            addFilterEvent.setValue(Resource.success(simpleApiResponse, simpleApiResponse.getMsg()));
                        } else {
