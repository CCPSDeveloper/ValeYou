...
public class ChatUsersAdapter extends RecyclerView.Adapter<ChatUsersAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        holder.holderChatListBinding.userName.setText(model.getChat().getUser().getName());
        holder.holderChatListBinding.message.setText(model.getChat().getMessage());
        ImageViewBindingUtils.setProfileUrl(holder.holderChatListBinding.userImage,"http://3.17.254.50:4999/upload/"+model.getChat().getUser().getImage());
        ...
        holder.holderChatListBinding.time.setText(convertTimeStampIntoDateTime(model.getChat().getCreatedAt()));
