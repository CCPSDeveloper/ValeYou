...
public class AddIdentityActivityVM extends BaseActivityViewModel {
    ...
    public void addIdentity(String auth_key, RequestBody type,RequestBody api_type, MultipartBody.Part frontImage, MultipartBody.Part backImage){
        welcomeRepo.addIdentity(auth_key,type,api_type,frontImage,backImage).subscribeOn(Schedulers.io())
    ...
    public void editIdentity(String auth_key,  RequestBody identity_id,RequestBody type,RequestBody api_type, MultipartBody.Part frontImage, MultipartBody.Part backImage){
        welcomeRepo.editIdentity(auth_key,identity_id,type,api_type,frontImage,backImage).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<SimpleApiResponse>() {
            @Override
            public void onSubscribe(Disposable d) {
                compositeDisposable.add(d);
