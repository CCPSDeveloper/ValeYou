...
public class ActivityIdentityVM extends BaseActivityViewModel {
    ...
    public void addIdentity(String auth_key, MultipartBody.Part image,MultipartBody.Part backImage){
        welcomeRepo.addIdentity(auth_key,image,backImage).subscribeOn(Schedulers.io())
    ...
    public void editIdentity(String auth_key, Map<String, RequestBody> map,MultipartBody.Part image,MultipartBody.Part backImage){
        welcomeRepo.editIdentity(auth_key,map,image,backImage).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(new SingleObserver<SimpleApiResponse>() {
            @Override
            public void onSubscribe(Disposable d) {
                compositeDisposable.add(d);
